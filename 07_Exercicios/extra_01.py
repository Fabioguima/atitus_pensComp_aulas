def letra_em_texto(texto, letra):
   ...
   try:
       texto.index(letra)
       print(f"A letra --> {letra} <-- existe no texto.")
   except:
       print(f"A letra --> {letra} <-- não existe no texto.")
...
print(texto_com_letra_upper("Pensamento Computacional", "a") == "PensAmento ComputAcionAl")